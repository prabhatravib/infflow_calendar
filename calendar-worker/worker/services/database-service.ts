...
>>>>>>> 7d9f3f4f91a2b718269f0ce8a4d10767a45ef837
  ...
  private async createDemoEvents(calendarId: string): Promise<void> {
    ...
    for (const event of demoEvents) {
      await this.db.prepare(`
        INSERT INTO events (id, calendar_id, title, description, start, end, tz, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).bind(event.id, event.calendar_id, event.title, event.description, event.start, event.end, event.tz, now.toISOString(), now.toISOString()).run();
    }
  }
}
