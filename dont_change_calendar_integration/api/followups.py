...
# ---------------------------------------------------------------------
# Reset/clear stored echo for an event chain
# ---------------------------------------------------------------------
@followups_bp.post("/events/<string:event_id>/echo/reset")
@require_args("user_id")
def reset_echo(event_id: str):
    """Clear the stored Mermaid flowchart on the parent event and all child
    echo events belonging to the same chain. The caller can pass either the
    parent event_id or any child echo event_id.

    This ensures the UI will stop auto-rendering an old flowchart until a new
    echo is generated.
    """
    ...
    try:
        ...
        # Try to find the parent event directly by id
        parent_event = None
        for ev in events:
            if isinstance(ev, dict) and str(ev.get("event_id") or ev.get("id")) == str(event_id):
                parent_event = ev
                break

        # If not found, see if the provided id is a child and locate its parent
        if not parent_event:
            for ev in events:
                if not isinstance(ev, dict):
                    continue
                child_ids = ev.get("echo_event_ids") or []
                if any(str(cid) == str(event_id) for cid in child_ids):
                    parent_event = ev
                    break
        ...
        parent_id = str(parent_event.get("event_id") or parent_event.get("id"))
        child_ids = list(parent_event.get("echo_event_ids") or [])

        # Clear flowchart on parent and unlink children
        try:
            client.update_event(parent_id, {"flowchart": "", "echo_event_ids": []})
        except Exception as e:
            logger.warning(f"Failed to clear flowchart on parent {parent_id}: {e}")

        cleared_children: list[str] = []
        for cid in child_ids:
            try:
                updated = client.update_event(str(cid), {"flowchart": ""})
                cleared_children.append(str(updated.get("event_id") or updated.get("id")))
            except Exception as e:
                logger.info(f"Skip clearing child {cid}: {e}")
        ...
        logger.error(f"Unexpected error in reset_echo: {str(e)}")
